...
with app.app_context():
    ...
    def postList(category):
        topics = ForumPost.query.filter_by(tag=category).order_by(ForumPost.time())
        return render_template("selectPost.html", topics=topics)


    @app.route('/post/<postID>', methods=['GET'])
    def replyToPost(postID):
        topic = ForumPost.query.filter_by(id = postID).first()
        replies = ForumReply.query.filter_by(id=postID).order_by(ForumReply.time())
        return render_template("selectPost.html", postID=postID, title=topic, replies=replies)
